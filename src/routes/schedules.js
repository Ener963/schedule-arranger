...
app.post('/', scheduleFormValidator, async (c) => {
  ...
  const { scheduleId } = await prisma.schedule.create({
    data: {
      ...
      scheduleName: body.scheduleName.slice(0, 255) || '(名前未設定)',
      memo: body.memo,
      createdBy: user.id,
      updatedAt: new Date(),
    },
...
app.get('/:scheduleId', scheduleIdValidator, async (c) => {
  ...
  // 各候補日程に対する各ユーザの出欠を入れ子の Map に格納するための Map を作る。
  ...
  // 閲覧ユーザと、出欠を登録したユーザ情報を書くのするための Map を作る。
...
app.post('/:scheduleId/update', scheduleIdValidator, async (c) => {
  const { user } = c.get('session') ?? {};
  const schedule = await prisma.schedule.findUnique({
    where: { scheduleId: c.req.valid('param').scheduleId },
  });
